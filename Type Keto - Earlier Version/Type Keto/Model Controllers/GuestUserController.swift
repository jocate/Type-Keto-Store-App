...
class GuestUserController {
    ...
    func createGuestUser(withName name: String, andEmail email: String, andOrderID orderID: String) -> GuestUser {
        orders.append(orderID)
        let guestUser = GuestUser(emailAddress: email, fullName: name, orderIds: orders)
        
        putGuestUserToServer(guestUser: guestUser)
        
        return guestUser
    }
    
    
    func putGuestUserToServer(guestUser: GuestUser, completion: @escaping (Error?) -> Void = {_ in }) {
        
        let identifier = guestUser.moltinUID 
        
        let urlPlusItem = baseURL.appendingPathComponent("guestusers")
        ...
        do {
            ...
            let productJSON = try encoder.encode(guestUser)
        ...
        URLSession.shared.dataTask(with: request) { (_, _, error) in
            if let error = error {
                NSLog("Error putting guest user to the server: \(error)")
                completion(error)
                return
            }
